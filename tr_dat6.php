...
<?php
    ...
    $sql = "UPDATE tramites SET Estado=3 WHERE IdTramite=".$idTramite ."";
       
    if ($conexion->query($sql) == TRUE) {
        $sql1 = "INSERT INTO tramitesObservaciones(Id, IdTramite, Fecha, Hora, NitavuCaptura, Observacion, Cancelado,Estado) 
        VALUES ( '', '$idTramite', '$fecha','$hora','$nitavu','$obs', 0,3)";        
        if ($conexion->query($sql1) == TRUE){
            historia($nitavu, "Marque como rechazado el tramite ".$idTramite." y puse esta observación ".$obs."");
            ...
            ' fue <B>RECHAZADO</B>, con las siguientes observaciones:<br>'.$obs.
            ...
            notificacion_add ($nitavuCaptura, 'El tramite '.$nombreTramite.' folio '.$idTramite.' fue RECHAZADO', date('Y-m-d'),$nitavu, $msgNoti);    
            echo 'TRUE';  
        }else{
            echo 'FALSE';
        }
