...
$Empleado = str_replace(" ", "", nitavu_nombre($IdEmpleado)); 
    ...
    $r= $conexion -> query($sql);if($f = $r -> fetch_array()){	
        ...
        $archivohost = $host.$f['File'];
        // echo $archivohost;
        $nuevo_nombre = $FechaNominaString.'_MiNomina_'.$Empleado.'.xml'; //asignamos nuevo nombre
        ...
        {
        ob_end_clean();
        header('Content-type:aplication/xml'); //Acá le cambias el tipo de archivo (MimeType) por lo que quieras
