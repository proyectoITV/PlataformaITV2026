...
$id_aplicacion ="ap09"; //Id de la aplicacion a cargar
//if (sanpedro($id_aplicacion, $nitavu)==TRUE){
	echo "<div id='AppDetalle'>".app_detalle($id_aplicacion, $nitavu)."</div>";
	// span ocupa 100% y Div 50%;

	xd_update('ap09',$nitavu);//guarda la experiencia del usuario
	historia($nitavu, "Entro a la aplicacion [ap09], Enviar documento electronico a todo un departamento ");


	echo "<form action='documentar_dpto_enviar.php' method='POST'>";
	echo "<input type='hidden' name='nitavu_' value='".$nitavu."'>";
	
	echo "<div>";
	echo "<label>Asunto:</label>";
	echo "<input type='hidden' name='dpto' value='".nitavu_dpto($nitavu)."'>";
	...
	echo "<textarea name='contenido' required='required'></textarea>";
	echo "</span>";

	echo "<div>";

		echo "<div><input type='submit' value='Enviar Doc. Electronico No. ".docdigital_no(TRUE, 0)."' class='Mbtn btn-default'></div>";
	echo "</div>";
