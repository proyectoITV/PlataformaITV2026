...
if (guia_proveedor($guia)=='1'){
    ...
    { 
        ...
        //  $pdf->write1DBarcode($guia, 'S25+', '', '', '', 24, 0.8, $stylebar, 'N');
        // $pdf->write1DBarcode($guia, 'C39', '', '', '', 24, 0.7, $stylebar, 'N');
        // $pdf->write1DBarcode($guia, 'C93', '', '', '', 24, 0.9, $stylebar, 'N');
        $pdf->write1DBarcode($guia, 'C128', '', '', '', 24, 0.9, $stylebar, 'N');       
        
        //$pdf->SetFont('', '', 8, '', 'false'); $pdf->Text(10, 40, 'Type Code Bar'. 'C128'); 
        

        $pdf->SetFont('', 'B', 10, '', 'false');
        $pdf->Text(80, 39, ''.$fecha); 
        ...
        $pdf->SetFont('', 'B', 7, '', 'false'); $pdf->Text(26, 108, ''.$f['origen_nombre']); 
        ...
        if ($f['origen_tipo']=='del'){//si es delegacion usamos el de la delegacion
            ...
            $pdf->Text(6, 118, ''.substr($f['origen_domicilio'],0,60)."--"); 
            $pdf->Text(6, 122, ' -- '.substr($f['origen_domicilio'],60)."."); 
        }else{//sino es del usamos el de oficinas centrales
            $pdf->SetFont('', '', 8, '', 'false');
            $pdf->Text(6, 118, ''.$pyme_direccion);
            $pdf->Text(6, 122, ''.$pyme_direccion2);
        }
        
        //======  ORIGEN TELEFONO ========
        if ($f['origen_tipo']=='del'){//si es delegacion usamos el de la delegacion
            $pdf->SetFont('', 'B', 10, '', 'false');
            $pdf->Text(70, 129, ''.$f['origen_telefono']); 
        }else{//sino es del usamos el de oficinas centrales
            $pdf->SetFont('', '', 10, '', 'false');
            $pdf->Text(70, 129, ''.$pyme_tels);
        ...
        $pdf->SetFont('', 'B', 12, '', 'false');
        $pdf->Text(45, 138, ''.$destino_ciudad); 
        ...
        if ($f['foraneo']=='1'){
            $pdf->Text(38, 149, ''.$f['destino']); 
            $pdf->SetFont('', 'B', 9, '', 'false'); ; 

        }else {
        $pdf->Text(38, 149, ''.$origen_remitente); 
        $pdf->SetFont('', 'B', 7, '', 'false'); $pdf->Text(38, 152, ''.$f['destino_nombre']); }

            
        //====== DOMICILIO DESTINO ========
        if ($f['foraneo']=='1'){
             $pdf->SetFont('', '', 9, '', 'false');
            $pdf->Text(6, 162, ''.substr($f['domicilio'],0,60)."--"); 
            $pdf->Text(6, 166, ' -- '.substr($f['domicilio'],60)."."); 
        }
        else{
        if ($f['destino_tipo']=='del'){//si es delegacion usamos el de la delegacion
            $pdf->SetFont('', '', 9, '', 'false');
            $pdf->Text(6, 162, ''.substr($f['destino_domicilio'],0,60)."--"); 
            $pdf->Text(6, 166, ' -- '.substr($f['destino_domicilio'],60)."."); 
        }else{//sino es del usamos el de oficinas centrales
            $pdf->SetFont('', '', 8, '', 'false');
            $pdf->Text(6, 162, ''.$pyme_direccion);
            $pdf->Text(6, 166, ''.$pyme_direccion2);
        }
    }
        //======  ORIGEN TELEFONO ========
        if ($f['foraneo']=='1'){
            $pdf->SetFont('', 'B', 10, '', 'false');
            $pdf->Text(46, 177, ''.$f['telefono']); 
        }
        else {
        if ($f['destino_tipo']=='del'){//si es delegacion usamos el de la delegacion
            $pdf->SetFont('', 'B', 10, '', 'false');
            $pdf->Text(46, 177, ''.$f['destino_telefono']); 
        }else{//sino es del usamos el de oficinas centrales
            $pdf->SetFont('', '', 10, '', 'false');
            $pdf->Text(46, 177, ''.$pyme_tels);
        ...
        if ($f['destino_tipo']=='del'){//si es delegacion usamos el de la delegacion
            $pdf->SetFont('', 'B', 10, '', 'false');
            $pdf->Text(94, 177, ''.$f['destino_cp']); 
        }else{//sino es del usamos el de oficinas centrales
            $pdf->SetFont('', '', 10, '', 'false');
            $pdf->Text(94, 177, ''.$pyme_cp);
        ...
        $pdf->SetFont('', '', 9, '', 'false');
        $pdf->Text(10, 192, 'Descripcion del envio:'); $pdf->Text(10, 198, ''); 
        $pdf->writeHTML('<p style="'.$recuadro4.'">'.$f['descripcion'].'</p>', true, false, true, false, '');
