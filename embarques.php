...
if (sanpedro($id_aplicacion, $nitavu) == TRUE) {
	...
	if ($nivel == 1) {
		submenu_add('?permisos', 'asistencia.png', 'Administrar', 'Permisos');
	}
	if ($nivel == 1) {
		submenu_add('?guias', 'guias.png', 'Administrar', 'Guias');
	}
	if ($nivel == 1) {
		submenu_add('?estadistica', 'embarques_estadisticar.png', 'Estadisticas', '');
	}
	...
	if (isset($_GET['personal'])) { //MODULO CARTERO //--------------------------------------------------------
		...
		if (isset($_POST['CarteroGuardar'])) {
			$npase = $_POST['npase'];
			if (paselibre($npase) == FALSE) {//solicitamos nuevamente un numero de pase
				$npase = npase(FALSE);
			}
			...
			if ($conexion->query($sql) == TRUE) {
				...
				if ($okPase == TRUE) {
					historia($nitavu, "Solicito Pase de Salida para entregar paqueteria " . $_POST['destino'] . " para " . nitavu_nombre($_POST['cartero']) . " con NPASE: " . $npase);
				}
				...
				if ($_POST['cartero2'] == '') {
					...
					if ($okPase2 == TRUE) {
						historia($nitavu, "Solicito Pase de Salida para entregar paqueteria " . $_POST['destino'] . " para " . nitavu_nombre($_POST['cartero2']) . " con NPASE: " . $npase);
					}
				...
				mensaje("ERROR: al guardar envio con personal de itavu:<br>Toma una foto o imprime pantalla, e informa al Dpto. de Informatica.<br>" . $sql, '');
			}
		...
		echo "<th>NPase</th>";
		echo "<th class='pc'>Asignacion</th>";
		echo "<th>Destino</th>";
		echo "<th class='pc'>Descripcion</th>";
		echo "<th class='pc'>Recibido</th>";
		echo "<th></th>";
		$ed = "";
		//echo $sql;
		$r2 = $conexion->query($sql);
		...
		if (isset($_POST['recibido_g'])) {
			if (!empty($_FILES['file_recibido']['name']) && !empty($_FILES['file_recibido']['tmp_name'])) {
				if ($_FILES["file_recibido"]["type"] == "image/jpeg") {//que solo acepte imagen			
					...
					$imagen = "";
					if (subir('file_recibido', 'img_embarques/' . $_POST['NPase'] . '', 'jpg') == 'Archivo subido con exito.!!') {
						$imagen = 'Imagen subida con exito';
					}
				...
				mensaje("ERROR: al recibir el archivo", 'embarques.php?personal');
			}
	...
	if (isset($_GET['guias']) and ($nivel == '1') and !isset($_GET['foraneo'])) {
		...
		if (isset($_POST['actualizar_guia'])) {//ACTUALIZAR GUIA------------
			...
			if ($asignacion == '' or $asignacion == 'FALSE') {// sin titular y sin permisos
				mensaje("No se puede asignar porque no hay nadie con permiso en dicho dpto, o no tiene un titular. " . $_POST['asignacion'], 'embarques.php?act=' . $_POST['guia']);
			} else {
				...
				if ($conexion->query($sql) == TRUE) {
					...
					mensaje("Guia " . $_POST['guia'] . " actualizada correctamente", 'embarques.php?guias=');
				} else {
					historia($nitavu, "ha habido un ERROR en la aplicacion de EMBARQUES al intentar actualizar. " . $sql);
					mensaje("Error al intentar actualizar: " . $sql, '');
		...
		if (isset($_POST['guardar_guia'])) {//GUARDAR GUIA NUEVA			-------------			
			$sql = "SELECT * FROM embarques_guias WHERE guia='" . $_POST['guia'] . "'";
			$rc = $conexion->query($sql);
			if ($rc && $f = $rc->fetch_array()) {
			...
			} else {//si no esta podemos agregarlo
				$asignacion = ''; //asignacion = titular y si no lo hay alguien que tenga permiso admin 2
				$asignacion = titular($_POST['asignacion']);
				...
				if ($asignacion == '' or $asignacion == 'FALSE') {// sin titular y sin permisos
					...
					$sql = "INSERT INTO embarques_guias (guia, asignacion, registro, registro_fecha, registro_hora, paqueteria_id, token, origen, destino, descripcion, recibido, recibio_fecha, recibio_hora, visto_paraimprimir, foraneo, ciudad, domicilio, telefono, autorizo) 

					VALUES ('" . $_POST['guia'] . "', '" . $asignacion . "','" . $nitavu . "', '" . $fecha . "', '" . $hora . "', '" . $_POST['proveedor'] . "','" . token_correo(FALSE) . "','" . $_POST['asignacion'] . "', '', '', '', '0000-00-00', '00:00:00', 0, 0, '', '', '', '')";
		...
		if ($nivel == '2' or $nivel == '1') {//si el encargado de embarques le dio permisos
			...
			while ($f = $r2->fetch_array()) {
				...
				if ($f['destino'] == '') {
					...
					echo "<a target:_blank href='plantilla_serumse.php?guia=" . $f['guia'] . "&nuc=$nitavu'><img src='icon/embarques_print.png' style='width:20px; height:20px; background-color:gray; border-radius:3px;'></a>";
				}
	...
	if (!isset($_GET['guias']) AND !isset($_GET['permisos']) AND !isset($_GET['estadistica']) AND !isset($_GET['enviar']) AND !isset($_GET['enviarforaneo']) AND !isset($_GET['personal'])) {
		if (isset($_POST['descripcion'])) {//actualizamos la descripcion si se ha recibido
			...
			if ($conexion->query($sql) == TRUE) {
				...
				mensaje("ERROR Al actualizar la descripcion de la guia " . $_POST['guia'] . "(" . $sql . ")", 'embarques.php');
			}
		}
		...
		if (isset($_GET['full'])) {
			$sql = "SELECT * FROM embarques_guias WHERE destino<>'' and origen='" . $midpto . "' and  YEAR(registro_fecha) = YEAR(NOW())  order by recibido";
		} else {
			$sql = "SELECT * FROM embarques_guias WHERE destino<>'' and origen='" . $midpto . "'  and  YEAR(registro_fecha) = YEAR(NOW()) and recibio_fecha='' order by recibido";
		}
		// if ($nitavu == '2809'){
		...
		while ($f = $r2->fetch_array()) {
			echo "<tr>";
			if ($f['recibido'] == '') {
				echo "<td  style='background-color:#C8DA92; ' width=20%>";
			} else {
				echo "<td width=40px>";
			}
			...
			if ($f['recibido'] == '') {
				echo "<td width=30% style='background-color:#C8DA92; font-size:9pt; '   valign=top align=center>";
			} else {
				echo "<td  style='font-size:9pt;' valign=top align=center>";
			}
			if ($f['foraneo'] == '1') {
				echo "Asignada como envio Foraneo<br>";
				echo "Destino: " . $f['destino'] . "<br>";
			...
			if ($f['registro_fecha'] <> '0000-00-00') {
				echo "<label style='font-size:8pt;'>Salio el " . fecha_larga($f['registro_fecha']) . " a las " . hora12($f['registro_hora']);
			}
			echo "</td>";


			if ($f['recibido'] == '') {
				echo "<td style='background-color:#C8DA92; font-size:9pt; '   valign=top align=center>";
			} else {
				echo "<td  style='font-size:9pt;' valign=top align=center>";
			}
			echo "" . nitavu_nombre($f['recibido']);
			if ($f['recibio_fecha'] <> '0000-00-00') {
				echo "<label style='font-size:8pt;'> Recibido el " . fecha_larga($f['recibio_fecha']) . " a las " . hora12($f['recibio_hora']) . "</label>";
			}
