 ...
 //fin nuevo form
    ...
    if (isset($_GET['busqueda'])){
        ...
        if (is_numeric($_GET['busqueda'])){
            ...
            $sqlreporte='Select numcertificado as NumCertificado, autorizo_fecha as Fecha, contrato as Contrato, Nombre, Curp, descripcion as Descripcion, Cantidad as Subsidio,
            (
            select fer_descripcion from cat_fer where fer.nfer_concepto=fer_IdCon
            )  as Concepto
            from fer
           where autorizo_fecha>="'.FechaInicioAdmin().'" and autorizo_fecha<="'.FechaTerminaAdmin().'" and estado=0  order by ejercicio, cast(nfer_id as INTEGER)';
        ...
        //'select * from fer where estado=0 and ejercicio='.$ejercicio.' 
        $sql = 'select * from fer where estado=0 and 
       autorizo_fecha>="'.FechaInicioAdmin().'" and autorizo_fecha<="'.FechaTerminaAdmin().'" and estado=0
        order by ejercicio, cast(nfer_id as INTEGER)';

        //echo $sql;
        //order by ejercicio, nfer_id";
        //echo "<h4>Ejercicio ".$ejercicio."</h4>";
        echo "<h4>Administración  ".date_format(date_create(FechaInicioAdmin()), 'd-m-Y')." al ".date_format(date_create(FechaTerminaAdmin()), 'd-m-Y')." </h4>";
        historia($nitavu, "Vio el ejercicio de ".$ejercicio." en FER");
    }
    //$sqlreporte='Select numcertificado as NumCertificado, autorizo_fecha as Fecha, contrato as Contrato, Nombre, Curp, descripcion as Descripcion, Cantidad as Subsidio,
    $sqlreporte='Select ejercicio as Ejercicio, numcertificado as NumCertificado, autorizo_fecha as Fecha, contrato as Contrato, Nombre, Curp, descripcion as Descripcion, Cantidad as Subsidio,
    (
    select fer_descripcion from cat_fer where fer.nfer_concepto=fer_IdCon
    )  as Concepto
    from fer
    where estado=0 and 
    autorizo_fecha>="'.FechaInicioAdmin().'" and autorizo_fecha<="'.FechaTerminaAdmin().'" order by ejercicio, cast(nfer_id as INTEGER)';
    ...
    guardareporte(1,$sqlreporte,$nitavu,'','',$ejercicio,'','produccion_itavu' );
            ...
            while($f = $r -> fetch_array()) {
                ...
                echo "<tr >";
                    ...
                    echo "<td id='ejercicio' name='ejercicio' >".$f['ejercicio']."</td>";
                       ...
                       /*  $d = $d."<p style='font-size:10pt;'><b class='normal'>".$f['nombre']."</b><br><b>CURP:</b> ".$f['curp']."<br>";
                        ...
                        //echo "<div id='modalModif".$f['nfer_id']."' class='MyModal'>";
                            if(isset($_POST['btnGuardarM'])){
                                ...
                                $ejerciciocert=$_POST['ejercicio'];
                                    echo" entre a modificar";
                                //----
                                               ...
                                               // if ($fecha <= FER_fechafin()){
                                                    ...
                                                    // if (FER_curp($curp)==""){
                                                            ...
                                                            , parrafo_opcional = '".$parrafo_opcional."', idempmodifica = '".$nitavu."', fechaultimamod = '".$fecha."'
                                                             where  ejercicio='".$ejerciciocert."'  and nfer_id = ".$idcertificado;
                                                             //where  ejercicio=2025  and nfer_id = ".$idcertificado;
                                                            ...
                                                            if ($conexion->query($sql) == TRUE){ 
                                                                mensaje('Se ha modificado el certificado con éxito'.$sql,'fer.php');
                                                              //  mensaje('Se ha modificado el certificado '.$idcertificado.' con éxito','fer.php');
                                                            }else{
                                                                mensaje('Ocurrio un error al intentar modificar el registro, favor de intentarlo nuevamente.'.$sql,'fer.php');
                                                             }
                                ...
                                echo "<center><table style='width:90%; style='font-size: 12;'>";
                                        echo "<tr>
                                             ...
                                             <label style='color:darkred; width:80px'>Ejercicio:</label><input type='text' id='ejercicio' name='ejercicio' value='".$f['ejercicio']."' style='width:80px' readonly >";
                                        ...
                                        echo "<td >    <span>
                                            ...
                                            <label style='color:darkred; '>Nombre del Beneficiario:</label>
                                            <input type='text' placeholder='' id='beneficiario' name='beneficiario' style='height: 30; font-size: 13; width:60%' value='".$f['nombre']."' required >        
                                        </span></td>
                                        </tr>
	                ...
	                //fin modal modificar
                      ...
                      //  echo $d;
                        ...
                        // hoy 12 mayo 2025 elimine bloqueo de año por instruccion lic. paty treto
                        ...
                        //if ($f['ejercicio']== date("Y", time()) ){ 
                            if ($f['estado'] == 0 and $nivel==1){
                                ...
                                echo " <a href='fer_print.php?print=".$f['nfer_id']."&ejerciciocert=".$f['ejercicio']."' class='Mbtn btn-secundario'><img src='icon/imprimir.png' style='width:15px;'></a>";
