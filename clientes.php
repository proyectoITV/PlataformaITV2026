...
echo "cliente nuevo";
	...
	if (isset($_GET['x'])){ //usario seleccionado
		...
		{//mostrar pantalla para actualizar datos del cliente
			...
			echo "<div><input type='submit' name='submit_act' value='Guardar' class='Mbtn btn-default'></div>";
	...
	if (isset($_GET['q'])){	//ejecutamos la consulta
		...
		while($fr = $r2 -> fetch_array())
		{
			...
			echo "<td width=50px>"; //acciones
				echo "<a class='Mbtn btn-default' style='display:block;' href='?x=".$fr['curp']."' style='text-decoration:none '><img src='icon/entrar.png'></a>";
			echo "</td>";
		...
		echo "<hr>";
		echo "<p>Si no encontro al cliente, puede <a class='Mbtn btn-default' href='clientes.php?nuevo='> Registrarlo </a> y asi agregarlo a su base de datos</p>";
	}
