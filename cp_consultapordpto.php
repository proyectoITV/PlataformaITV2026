...
if(isset($_GET['dpto'])){
    ...
    if($caso==1){
        ...
        AND estado = 1) as t1 where turnadoa = ".$dpto." ORDER BY id DESC";
        }else{
            $query = "select * from (SELECT DISTINCT * FROM cp_nuevosdocumentos 
        ...
        if ($nivel==1 || soytitular($nitavu)!='FALSE'){
            $query = "select * from (SELECT DISTINCT * FROM cp_nuevosdocumentos 
        WHERE (nitavuCaptura = ".$nitavu." OR idDptoCrea = ".nitavu_dpto($nitavu)."  OR id IN (SELECT numcaso FROM cp_colaboradores WHERE nitavu=".$nitavu.") OR 
        id IN (SELECT NumCaso FROM cp_historialdocumentos WHERE dptoSube=".nitavu_dpto($nitavu)." OR dptoEnviar=".nitavu_dpto($nitavu).") OR turnadoa = ".nitavu_dpto($nitavu)." OR id IN (SELECT CasoId FROM cp_comentarios WHERE Nuser = ".$nitavu."))
        AND estado = 0) as t1 where  turnadoa = ".$dpto." ORDER BY id DESC";
    ...
    //**********DIV PARA IMPRIMIR****************/
    echo "<div id='imprimir4' style='display:none;'>";
    //style='display:none;'
    ...
    echo "</div>";
        ...
        $cont=0;
            ...
            echo '<a style="position: absolute; top: 50; right: 0;" type="button" id="btn" value="Print" onclick="printDiv(4)" ><img src="icon/pdf.png" style="width:25px;"></a>';
