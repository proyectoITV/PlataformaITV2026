<?php 
 ...
 require_once("lib/flor_funciones.php");
    ...
    $IdCategoria = $_POST['IdCategoria']; if (ValidaVAR($IdCategoria)==TRUE){$IdCategoria = LimpiarVAR($IdCategoria);} else {$IdCategoria = "";}
    ...
    $tipoInfo = $_POST['tipoInfo']; if (ValidaVAR($tipoInfo)==TRUE){$tipoInfo = LimpiarVAR($tipoInfo);} else {$tipoInfo = "";}
    $Usuario = $nitavu;
    $type = SolicitudRequisitoTipo($IdRequisito);
    if ($type == 'file'){
		...
		if ($_FILES[$IdRequisito]['error'] !== 0) {
            ...
            if ( mime_content_type($_FILES[$IdRequisito]['tmp_name']) == 'application/pdf')
            {
                if (move_uploaded_file($ArchivoTmp, $NombreDelArchivo)) { //se subio correctamente
                    //Guardar Dato
                    GuardarSolicitudDato($FolioTramite, $IdRequisito, $NombreDelArchivo, "file", $nitavu, $IdCategoria,$tipoInfo); //<--- guardamos el nombre del archivo como dato
        ...
        if (GuardarTramiteDato($FolioTramite, $IdRequisito, $Valor, "", $Usuario, $IdCategoria) == FALSE) {
           ...
           $ti = "";  $type = SolicitudRequisitoTipo($IdRequisito, $IdCategoria);
           if ($type == 'select') {//Escribimos otra vez el select
             
               if($IdRequisito == 4){
                   if ($Valor <> '' ) {
                           $sql = "SELECT * FROM CatSexo";
               ...
               }else if($IdRequisito == 39 || $IdRequisito == 49 ){
                   if ($Valor <> '' ) {
               ...
               }else if($IdRequisito == 41){
                   if ($Valor <> '' ) {
                       $sql = "SELECT * from catcolonia";
                       $r2x = $Vivienda -> query($sql);
                       $ti = $ti. '<option value="9999">SELECCIONE UNA OPCION...</option>';
                       while($fxx = $r2x -> fetch_array())
                       {
