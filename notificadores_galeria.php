...
//echo $munis;
historia($nitavu,"Uso la Galeria de Notificadores (".$id_aplicacion.")");
if ($munis=='FALSE')
{
   ...
   {// despliega las delegaciones del municipio
      ...
      FROM notificadores_visitas WHERE notificadores_visitas.delegacion = '".$f['id']."'  AND notificadores_visitas.vobo <>'' ORDER BY colonia";
      //echo $sqlx2;
            ...
            echo "<option value=''>Todas las colonias de ".municipio_nombre($_GET['m'])."</option>";
            echo "</select></div>";


            echo "<div><label>Estado de Lote:</label><select name='el'>";
            $rp= $conexion -> query("select * from cat_estado_lotes");  while($el = $rp -> fetch_array())            
            {  
               if (isset($_GET['el'])){
                  if ($el['id'] == $_GET['el']){
                  echo "<option value='".$el['id']."' selected='selected'>".$el['nombre']."</<option>";
                  } else {
                  echo "<option value='".$el['id']."'>".$el['nombre']."</<option>";
                  }
               } else {
                  echo "<option value='".$el['id']."'>".$el['nombre']."</<option>";//pone todas si no hay variable get
               }
   ...
   if (isset($_GET['col'])){
     ...
     echo "<div id='indicadores_full'>";
      $sql="
      SELECT
         notificadores_visitas.id_colonia as noti_id_colonia,
         notificadores_visitas.id_municipio as noti_id_municipio,
         (select cat_colonias.Colonia from cat_colonias where cat_colonias.IdColonia = noti_id_colonia AND cat_colonias.IdMunicipio = noti_id_municipio) as Colonia,
        (select cat_municipios.nombre from cat_municipios where cat_municipios.IdMunicipio = noti_id_municipio) as Municipio, 
         notificadores_visitas.visita_fecha,
         notificadores_visitas.contrato,
         notificadores_visitas.visita_comentarios,
         notificadores_visitas.id,
         notificadores_visitas.id_estado_lote as noti_id_estado_lote,
         notificadores_visitas.id_estado_ubv as noti_id_estado_uvb,
         (select cat_estado_lotes.nombre from cat_estado_lotes where cat_estado_lotes.id = noti_id_estado_lote) as estado,
         notificadores_visitas.transpaso as noti_transpaso,
         (select cat_transpasos.nombre from cat_transpasos where cat_transpasos.id = noti_transpaso) as transpaso_nombre,
         notificadores_visitas.manzana,
         notificadores_visitas.lote,
         notificadores_visitas.brigada_id,
      ...
      AND notificadores_visitas.vobo <> ''

      ";
      if (isset($_GET['el'])){
         if ($_GET['el']<>''){ $sql = $sql." AND notificadores_visitas.id_estado_lote = '".$_GET['el']."' ";}
      }
      
      if (isset($_GET['col'])){
         if ($_GET['col']<>''){
            $sql = $sql. " AND notificadores_visitas.id_colonia='".$_GET['col']."' AND notificadores_visitas.id_municipio='".$_GET['m']."' ";
         }
         
         if (isset($_GET['busqueda'])){
            historia($nitavu,"Busco ".$_GET['busqueda']."en la Galeria de Notificadores (".$id_aplicacion.")");
            $sql = $sql. " OR notificadores_visitas.visita_comentarios like'%".$_GET['busqueda']."%'";
            $sql = $sql. " OR notificadores_visitas.contrato like'%".$_GET['busqueda']."%'";
            $sql = $sql. " OR notificadores_visitas.nombre like'%".$_GET['busqueda']."%'";
            $sql = $sql. " OR notificadores_visitas.paterno like'%".$_GET['busqueda']."%'";
            $sql = $sql. " OR notificadores_visitas.materno like'%".$_GET['busqueda']."%'";
      ...
      { 
          if ($re['noti_id_estado_lote']=='1'){echo "<article class='baldio'>";}
            if ($re['noti_id_estado_lote']=='2'){echo "<article class='comercio'>";}
            if ($re['noti_id_estado_lote']=='3'){echo "<article class='habitado'>";}
            if ($re['noti_id_estado_lote']=='4'){echo "<article class='rentado'>";}
            if ($re['noti_id_estado_lote']=='5'){echo "<article class='construccion'>";}
            if ($re['noti_id_estado_lote']=='6'){echo "<article class='deshabitado'>";}
            if ($re['noti_id_estado_lote']=='7'){echo "<article class='mixto'>";} 
            if ($re['noti_id_estado_lote']==''){echo "<article>";} 
            ...
            } else {echo "<a href='".$ruta_deretorno."&full=".$re['id']."' title='Haz clic sobre la foto para maximizarla'><img src='$archivo_foto' class='foto_galeria'></a>";}
         
            //echo "<span class='tenue'>Col. <b class=''>".colonia_nombre($re['id_colonia'],$re['id_municipio'])."</b>".";
            echo "<b class='inversa_gris_claro'>m".$re['manzana']."</b> <b class='inversa_gris'>L".$re['lote']."</b><br></span>";
            
            echo "<span class='comentarios'>";
            echo "<label >Comentarios de la visita</b> (brigada ".$re['brigada_id']."): </label>";
            echo $re['visita_comentarios']."</span>";
            
            echo "<div class='tchico'>Col. ".$re['Colonia'].", ".$re['Municipio']."</div>";
            echo "<div class='seleccionada'>".$re['nombre']." ".$re['paterno']." ".$re['materno']."</div>";
            echo "<div class='tchico normal' >Contrato: ".$re['contrato']."</div>";

            echo "<b>Lote ".$re['estado']."  </b><br>";
            echo "<a class='Mbtn btn-secundario' href='
            notificadores_visita2.php?brig=".$re['brigada_id']."&col=".$re['noti_id_colonia']."&mun=".$re['noti_id_municipio']."&m=".$re['manzana']."&l=".$re['lote']."'>";
            ...
            $title= "Manzana ".$re['manzana']." Lote ".$re['lote'];
            $des = "Beneficiario: ".$re['nombre']." ".$re['paterno']." ".$re['materno']."<br>";
            $des = $des." Col. ".$re['Colonia']."<br>";
            $des = $des."Contrato: ".$re['contrato']."";
            $des = $des."<br><br>Visita hecha por <b>".nitavu_nombre($re['notificador_nitavu'])."</b> a las ".$re['visita_hora']." de ".$re['visita_fecha']." y Verificada (Vo.Bo.) Vo.Bo. por <b> ".nitavu_nombre($re['vobo'])."</b>";

            $div = "<h1>".$title."</h1><div>".$des."<div><img width=400px src=".$archivo_foto."><br> Clasificada como ".$re['estado'];
            historia($nitavu,"Vio ".$title.", ".$des." de  la Galeria de Notificadores (".$id_aplicacion.")");
            echo " <a class='Mbtn btn-secundario ' href='
            geomapa.php?lat=".$re['visita_lat']."&lon=".$re['visita_lon']."&title=".$title."&div=".$div."'>";
   ...
   echo "</div>";
       ...
       echo sugerencia("Una colonia esta disponible aquí, cuando ha sido verificada y revisada por delegacion; es decir que ha dado el Vo.Bo. La delegación correspondiente.");

       echo "<br><hr>";
       $buscar_url = 'notificadores_galeria.php?m='.$_GET['m'];
       if (isset($_GET['col'])) { $buscar_url = $buscar_url."&col=".$_GET['col'];}
       echo '<div id="beta_buscar">';
       echo '<form action="notificadores_galeria.php" method="get">';
       if (isset($_GET['brig'])){echo '<input type="hidden" name="brig" id="brig" value="'.$_GET['brig'].'">';}
       if (isset($_GET['col'])){echo '<input type="hidden" name="col" id="col" value="'.$_GET['col'].'">';}
       if (isset($_GET['m'])){echo '<input type="hidden" name="m" id="m" value="'.$_GET['m'].'">';}

            echo '<table broder="1" width="100%"><tr>';
               echo '<td>                    <input type="text" id="beta_buscar_input" name="busqueda" placeholder="Buscar por nombre, comentario y contrato" /></td>';
