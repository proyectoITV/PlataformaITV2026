...
class TCPDF {
	...
	// --- variablesused for document encryption:
	 ...
	 */
	public function AddPage($orientation='', $format='', $keepmargins=false, $tocpage=false, $des) {
		if ($this->inxobj) {
	 ...
	 */
	public function endPage($tocpage=false, $des) {
	...
	public function Footer($des) {
		...
		if (empty($this->pagegroups)) {
			$pagenumtxt = $w_page.$des.' '.$this->getAliasNumPage().' / '.$this->getAliasNbPages();
		} else {
			$pagenumtxt = $w_page.$des.' '.$this->getPageNumGroupAlias().' / '.$this->getPageGroupAlias();
		}
		$this->SetY($cur_y);
		//Print page number
		if ($this->getRTL()) {
	 ...
	 * @since 4.0.012 (2008-07-24)
	 */
	protected function setFooter($des) {
		if ($this->state != 2) {
			return;
		}
		$this->InFooter = true;
