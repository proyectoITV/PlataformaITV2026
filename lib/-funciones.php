...
/*credenciales para consultar curp*/
...
function Nfer_new2($consulta){
	require("config.php");
	$ejercicio = date('Y');
	$sql = "SELECT MAX(nfer_id)+1 as Nfer  FROM fer where ejercicio='".$ejercicio."'" ;
	$rc= $conexion -> query($sql);
	if($f = $rc -> fetch_array())
					{
		if ($consulta==TRUE) {
...
function Ncert_new($consulta){
	require("config.php");
	$ejercicio = date('Y');
	$sql = "SELECT MAX(numcertificado)+1 as nuevocert FROM fer where estado=0 and ejercicio='".$ejercicio."'"  ;
	$rc= $conexion -> query($sql);
	if($f = $rc -> fetch_array())
					{
		if ($consulta==TRUE) {
