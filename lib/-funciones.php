<?php
//Departamento de Informática | ITAVU
	...
	if ($f = $rc->fetch_array()) {
		...
		if ($var1 == 1) {
			if (isset($_GET['var1'])) { // si detectamos el valor de var1			
				$var1_str = $_GET['var1'];
				if (ValidaVAR($var1_str) == TRUE) {
					$var1_str = LimpiarVAR($var1_str);
				} else {
					$var1_str = "";
				}
	...
	if ($SQL_go == TRUE) {
		if (!empty($sql) == true) {
			...
			if ($rc2) {
				...
				while ($f2 = $rc2->fetch_row()) {//LISTAR COLUMNAS
	...
	if ($SQL_go == TRUE) {
		...
		if (!empty($sql) == true) {
			...
			if ($rc3) {
				...
				// $rc3 = $conexion -> query($sql);
				while ($f3 = $rc3->fetch_row()) {//LISTAR COLUMNAS
...
function RI_SQL1_interactive($id_rep, $Value_var1, $Value_var2, $Value_var3)
{
	...
	if ($Value_var2 <> '') { // si detectamos el valor de var1			
		$var2_str = $Value_var2;
		if (ValidaVAR($var2_str) == TRUE) {
			$var2_str = LimpiarVAR($var2_str);
		} else {
			$var2_str = "";
		}
	...
	if ($Value_var3 <> '') { // si detectamos el valor de var1			
		$var3_str = $Value_var3;
		if (ValidaVAR($var3_str) == TRUE) {
			$var3_str = LimpiarVAR($var3_str);
		} else {
			$var3_str = "";
		}
		...
		public function Footer()
		{
			if ($this->PageSize == "LETTER") { //Configuracion CARTA
				if ($this->orientacion == 'L') { //horizontal CARTA						
					...
					$this->Text(15, 201.5, $paginas);
					$this->SetFont('helvetica', 'I', 6);
					$this->Text(40, 201.5, "[" . $this->id_rep . "] " . substr($this->info_leyenda, 0, $LogitudTitulo) . "");
					$this->SetFont('helvetica', 'I', 6);
					$this->Text(40, 203.5, "" . substr($this->info_leyenda, $LogitudTitulo + 1, $LogitudTitulo) . "");
					...
					$LogitudTitulo = 150;
					$this->SetFont('helvetica', 'B', 9);
					$this->Text(15, 265, $paginas);
					$this->SetFont('helvetica', 'I', 6);
					$this->Text(40, 265, "[" . $this->id_rep . "] " . substr($this->info_leyenda, 0, $LogitudTitulo) . "");
					$this->SetFont('helvetica', 'I', 6);
					$this->Text(40, 267, "" . substr($this->info_leyenda, $LogitudTitulo + 1, $LogitudTitulo) . "");
				...
				if ($this->orientacion == 'L') { //horizontal OFICIO
					...
					$LogitudTitulo = 150;
					$this->SetFont('helvetica', 'B', 9);
					$this->Text(15, 327, $paginas);
					$this->SetFont('helvetica', 'I', 6);
					$this->Text(40, 327.5, "[" . $this->id_rep . "] " . substr($this->info_leyenda, 0, $LogitudTitulo) . "");
					$this->SetFont('helvetica', 'I', 6);
					$this->Text(40, 329.8, "" . substr($this->info_leyenda, $LogitudTitulo + 1, $LogitudTitulo) . "");
...
function VEdoCuentaTablaDePagos($contrato, $IdDelegacion, $ContratoCancelado, $OriginData, $full, $cancelados)
{
 ...
 ";
	...
	$GranCargo = 0;
	$GranAbono = 0;
	$styleERROR = "background-color:#FFCC99;";
	$Calculo_Saldo = 0;
	$c = 0;
	$Registros = 0;
	$rc = $Vivienda->query($sql7_count);
	if ($f = $rc->fetch_array()) {
		$Registros = $f['n'];
	}
	...
	$StringStrike = "";
	$StringStrike2 = "";
	if ($ContratoCancelado == 1) {
		$StringStrike = "<strike>";
		$StringStrike2 = "</strike>";
	}
...
function EdoCuentaPDF($contrato, $leyenda, $Delegacion, $FechaContrato, $Beneficiario, $t1, $cancelado, $txtQR)
{
	...
	class PDFEDOCUENTA extends TCPDF
	{
		...
		public function Header()
		{
			...
			$this->SetFont('courier', 'R', 7);
			$this->Text(85, 12, '');
			$this->SetFont('courier', 'B', 8);
			$this->Text(90, 12, 'Contrato: ' . $this->NumContrato);
			$this->SetFont('courier', 'R', 6);
			$this->Text(85, 15, '');
			$this->SetFont('courier', 'B', 6);
			$this->Text(100, 15, 'Fecha del Contrato:' . $this->FechaContrato);
...
function ConvertirFechaParaMySQL($strFecha)
{
	...
	$FechaConFormatoCompatible = str_replace('/', '-', $FechaNacimiento);
	$FechaConFormatoCompatible = date('Y-m-d', strtotime($FechaConFormatoCompatible));
	$FechaNacimiento = $FechaConFormatoCompatible;
...
function FormularioTramite($FolioTramite, $CURP, $IdTipoTramite, $Nombres, $Apellido1, $Apellido2, $Sexo, $FechaNacimiento, $StatusCurp, $Usuario, $nivel, $Nacionalidad, $EntidadNacimiento, $numEntidadNacimiento, $IdDelegacion, $IdPrograma)
{
	...
	if (issetTramite($FolioTramite) == 0) {// si es la primera vez
		...
		if ($conexion->query($sql) == TRUE) {
			...
			if (GuardarTramiteDato($FolioTramite, 0, $CURP, "text", $Usuario, 0) == FALSE) {
				echo "<script>console.log('Error al guadar Folio: " . $FolioTramite . ", IdRequisito: CURP');</script>";
			}
			if (GuardarTramiteDato($FolioTramite, 1, $Nombres, "text", $Usuario, 0) == FALSE) {
				echo "<script>console.log('Error al guadar Folio: " . $FolioTramite . ", IdRequisito: Nombres');</script>";
			}
			if (GuardarTramiteDato($FolioTramite, 2, $Apellido1, "text", $Usuario, 0) == FALSE) {
				echo "<script>console.log('Error al guadar Folio: " . $FolioTramite . ", IdRequisito:Apellido1');</script>";
			}
			if (GuardarTramiteDato($FolioTramite, 3, $Apellido2, "text", $Usuario, 0) == FALSE) {
				echo "<script>console.log('Error al guadar Folio: " . $FolioTramite . ", IdRequisito: Apellido2');</script>";
			}
			if (GuardarTramiteDato($FolioTramite, 4, $Sexo, "text", $Usuario, 0) == FALSE) {
				echo "<script>console.log('Error al guadar Folio: " . $FolioTramite . ", IdRequisito: sexo');</script>";
			}
			if (GuardarTramiteDato($FolioTramite, 5, date('d-m-Y', strtotime(str_replace('/', '-', $FechaNacimiento))), "date", $Usuario, 0) == FALSE) {
				echo "<script>console.log('Error al guadar Folio: " . $FolioTramite . ", IdRequisito: FechaNacimiento');</script>";
			}
			if (GuardarTramiteDato($FolioTramite, 8, $StatusCurp, "text", $Usuario, 0) == FALSE) {
				echo "<script>console.log('Error al guadar Folio: " . $FolioTramite . ", IdRequisito: StatusCURP');</script>";
			}

			if (GuardarTramiteDato($FolioTramite, 6, $Nacionalidad, "text", $Usuario, 0) == FALSE) {
				echo "<script>console.log('Error al guadar Folio: " . $FolioTramite . ", IdRequisito: Nacionalidad');</script>";
			}
			if (GuardarTramiteDato($FolioTramite, 7, $EntidadNacimiento, "text", $Usuario, 0) == FALSE) {
				echo "<script>console.log('Error al guadar Folio: " . $FolioTramite . ", IdRequisito: Entidad de Nacimiento');</script>";
			}
			if (GuardarTramiteDato($FolioTramite, 99, $numEntidadNacimiento, "text", $Usuario, 0) == FALSE) {
				echo "<script>console.log('Error al guadar Folio: " . $FolioTramite . ", IdRequisito: Entidad de Nacimiento');</script>";
			}
	...
	if ($Continuo == TRUE) { //<--- si puede continuar con el tramite
		...
		$rClase = $conexion->query($sqlClase);
		while ($fClase = $rClase->fetch_array()) {
						...
						if ($fClase['IdClase'] == 0 and $IdCategoria == 4) {
							...
							if ($f3['Cancelado'] == 0) {
								...
								switch ($type) {
									case "file":
										if ($value <> '') {
											$vinculo = "<a href='" . $value . "' download='MiRequisito" . $IdRequisito . ".pdf' target=_blank><img src='icon/pdf.png' style='width:36px;'></a>";
											if ($f3['Opcional'] == 0) {
...
function TomarTurno($nitavu)
{
	...
	if (MiTurnoActual($nitavu) == "") { // sino tengo turnos
		...
		if ($MisAreas == "") {
			...
			$rc = $conexion->query($sql);
			if ($f = $rc->fetch_array()) {	//actualizamos el estado
...
function Historia_UltimoMovimiento($nitavu)
{
	require("config.php");
...
function Historia_PrimerMovimiento($nitavu)
{
	require("config.php");
...
('$nitavu', '$xmlCont', '$FechaIni','$FechaFin','$autorizo','$fecha','$hora','$periodo')";
	if ($conexion->query($sql) == TRUE) {	//echo "ok";
		//notificamos
...
function org_json()
{
	...
	if ($conexion->query($sql) == TRUE) {
		$r2 = $conexion->query($sql = "SELECT * FROM cat_gerarquia WHERE id='0'");
		while ($f = $r2->fetch_array()) {
			if (org_dependencias($f['id']) == 0) {
				$j = $j . "{'name' : '" . $f['nombre'] . "', 'title': '" . nitavu_nombre($f['titular']) . "', 'className': '" . $f['nivel'] . "'},";
			} else {
				$j = $j . "'name' : '" . $f['nombre'] . "', 'title': '" . nitavu_nombre($f['titular']) . "', 'className': '" . $f['nivel'] . "', 'children':[";


				$r3 = $conexion->query($sql = "SELECT * FROM cat_gerarquia WHERE dependencia='" . $f['id'] . "'");
				while ($f3 = $r3->fetch_array()) {
					if (org_dependencias($f3['id']) == 0) {
						$j = $j . "{'name' : '" . $f['nombre'] . "', 'title': '" . nitavu_nombre($f3['titular']) . "', 'className': '" . $f3['nivel'] . "'},";
					} else {
						$j = $j . "{'name' : '" . $f3['nombre'] . "', 'title': '" . nitavu_nombre($f3['titular']) . "', 'className': '" . $f3['nivel'] . "', 'children':[";


						$r4 = $conexion->query($sql = "SELECT * FROM cat_gerarquia WHERE dependencia='" . $f3['id'] . "'");
						while ($f4 = $r4->fetch_array()) {
							if (org_dependencias($f4['id']) == 0) {
								$j = $j . "{'name' : '" . $f4['nombre'] . "', 'title': '" . nitavu_nombre($f4['titular']) . "', 'className': '" . $f4['nivel'] . "'},";
							} else {
								$j = $j . "{'name' : '" . $f4['nombre'] . "', 'title': '" . nitavu_nombre($f4['titular']) . "', 'className': '" . $f4['nivel'] . "', 'children':[";


								$r5 = $conexion->query($sql = "SELECT * FROM cat_gerarquia WHERE dependencia='" . $f4['id'] . "'");
								while ($f5 = $r5->fetch_array()) {
									if (org_dependencias($f5['id']) == 0) {
										$j = $j . "{'name' : '" . $f5['nombre'] . "', 'title': '" . nitavu_nombre($f5['titular']) . "', 'className': '" . $f5['nivel'] . "'},";
									} else {
										$j = $j . "{'name' : '" . $f5['nombre'] . "', 'title': '" . nitavu_nombre($f5['titular']) . "', 'className': '" . $f5['nivel'] . "', 'children':[";



										$r6 = $conexion->query($sql = "SELECT * FROM cat_gerarquia WHERE dependencia='" . $f5['id'] . "'");
										while ($f6 = $r6->fetch_array()) {
											if (org_dependencias($f6['id']) == 0) {
												$j = $j . "{'name' : '" . $f6['nombre'] . "', 'title': '" . nitavu_nombre($f6['titular']) . "', 'className': '" . $f6['nivel'] . "'},";
											} else {
												$j = $j . "{'name' : '" . $f6['nombre'] . "', 'title': '" . nitavu_nombre($f6['titular']) . "', 'className': '" . $f6['nivel'] . "', 'children':[";


												$r7 = $conexion->query($sql = "SELECT * FROM cat_gerarquia WHERE dependencia='" . $f6['id'] . "'");
												while ($f7 = $r7->fetch_array()) {
													if (org_dependencias($f7['id']) == 0) {
														$j = $j . "{'name' : '" . $f7['nombre'] . "', 'title': '" . nitavu_nombre($f7['titular']) . "', 'className': '" . $f7['nivel'] . "'},";
													} else {
														$j = $j . "{'name' : '" . $f7['nombre'] . "', 'title': '" . nitavu_nombre($f7['titular']) . "', 'className': '" . $f7['nivel'] . "', 'children':[";

														$r8 = $conexion->query($sql = "SELECT * FROM cat_gerarquia WHERE dependencia='" . $f7['id'] . "'");
														while ($f8 = $r8->fetch_array()) {
															if (org_dependencias($f8['id']) == 0) {
																$j = $j . "{'name' : '" . $f8['nombre'] . "', 'title': '" . nitavu_nombre($f8['titular']) . "', 'className': '" . $f8['nivel'] . "'},";
															} else {
	...
	if ($conexion->query($sql) == TRUE) {
		...
		if ($f = $rc->fetch_array()) {
			if ($f['titular'] == '') {//si no hay titular
				...
				if ($conexion->query($sql) == TRUE) {
					...
					if ($f2 = $rc2->fetch_array()) {
						if ($f2['titular'] == '') {//si este tampoco tiene titular vamos al siguiente
							...
							if ($conexion->query($sql) == TRUE) {
								...
								if ($f3 = $rc3->fetch_array()) {
									if ($f3['titular'] == '') {//si este tampoco tiene titular vamos al siguiente
										...
										if ($conexion->query($sql) == TRUE) {
											...
											if ($f4 = $rc4->fetch_array()) {
												if ($f4['titular'] == '') {//si este tampoco tiene titular vamos al siguiente
													...
													return $f4['titular']; //Damos el titular de este dpto en su nivel 3		
												}
											}
										}
										...
										return $f3['titular']; //Damos el titular de este dpto en su nivel 3		
									}
								}
							}
							...
							return $f2['titular']; //Damos el titular de este dpto
						}
				...
				return $f['titular']; //Titular de tu Dpto
			}
		}
	} else {
		return FALSE;
...
function ingresos_totales($IdDelegacion, $fecha_)
...
function notifi_sinleer($nitavu)
{ //NOTIFICACIONES, TOTALES
	require("config.php");
...
function notifi_enviadassinleer($nitavu)
{ //NOTIFICACIONES, TOTALES
	require("config.php");
...
function comida_aut($nitavu)
{ //consulta experiencia del usuario
	require("config.php");
...
function comida_salida2($nitavu, $fecha2)
{ //consulta experiencia del usuario
	require("config.php");
...
function comida_trestante($nitavu)
{ //consulta experiencia del usuario
	require("config.php");
	$sql = "select * from empleados_salidas_temporal where nitavu='" . $nitavu . "' and fecha='" . $fecha . "' and asunto='COMIDA'";
	$rc = $conexion->query($sql);
...
function xd_update($idap, $nitavu)
{//actualizar sd
	require("config.php");
	$sql = "SELECT	* from xd where idap='" . $idap . "' and iduser='" . $nitavu . "'";
	$rc = $conexion->query($sql);
	if ($f = $rc->fetch_array()) {//update
		$c = xd($idap, $nitavu) + 1;
...
function pendientes_($nitavu)
{
	...
	if (sanpedro($id_aplicacion, $nitavu) == TRUE) {
		...
		$pendientes = "";
		$c = 0;
		while ($f = $r2->fetch_array()) {//$df recorre la lista de las delegaciones
...
function notificadores_pendientes($nitavu)
{
	...
	if ($id_delegacion == '') {
		...
		$sql = "SELECT	count(*) as n FROM	notificadores_visitas WHERE	visitada = ''AND delegacion = '" . $id_delegacion . "'";
	}
	////echo $sql;
	$rc = $conexion->query($sql);
	$msg = "";
...
function misdelegaciones_conid($id)
{
	...
	$r2 = $conexion->query($sql2);
	$tmp = "";
	while ($df = $r2->fetch_array()) {//$df recorre la lista de las delegaciones
	...
	if ($midelegacion_lugar == 'Coordinacion') {
		...
		$midelegacion_id = busca_id_delegacion($midelegacion_lugar);
	}
...
function pase_estado($empleado, $desde, $hasta, $todos)
{
	...
	while ($f = $r->fetch_array()) { // resultado de la busqueda.................
		...
		if ($f['autorizo_nitavu'] == '') {
			...
			$aut = $m . "<br>Autorizado por " . nitavu_nombre($f['autorizo_nitavu']) . " a las " . $f['autorizo_hora'] . " a " . fecha_larga($f['autorizo_fecha']);
		}
...
(nitavu='" . $nitavu_ . "') ORDER by dpto ASC";
	...
	if ($detalles == 'TRUE') {
		if ($resumen <> "") {
			return "<strong class='alerta grande'>" . $total_retraso . " min.</strong><br><lu>" . $resumen . "</lu>";
		}
	} else {
...
function itop($ip)
{
	...
	$tmp = "";
	while ($f = $r2->fetch_array()) {//Categorias de Aplicaciones
...
//echo $contenido;
	...
	if ($usuario <> '') {
		$npase = npase(FALSE);
		$sql = "INSERT INTO notificaciones
	(nitavu, asunto, entregar_fecha, nitavu_manda, contenido, grupal, lectura_fecha, lectura_hora, pie, firma_img, docdigital, no_oficio, prefijo, visto)
VALUES
	('$usuario', '$asunto', '$entregar_fecha','$itavu_manda', '$contenido', '', '0000-00-00', '', '', '', '', '', '', 0)";
		////echo $sql;
		if ($conexion->query($sql) == TRUE) {
...
//pero a san pedro no le importa tu nivel, si estas en la lista te deja pasar
	...
	} else { //historia($usuario, "Se le nego el acceso a la aplicacion con ID ".$idapp); 
		if ($NoProblem == 0) {
...
function InfoEquipo()
{
	...
	$red = "";
	// if ($ip <> '' ){$red = $red."ip:".$ip;	}
	if (strlen(getenv('HTTP_CLIENT_IP')) > 3) {
		$red = $red . " " . getenv('HTTP_CLIENT_IP');
	}
	if (strlen(getenv('HTTP_X_FORWADED_FOR')) > 3) {
		$red = $red . ", " . getenv('HTTP_X_FORWADED_FOR');
	}
	if (strlen(getenv('REMOTE_ADDR')) > 3) {
		$red = $red . ", " . getenv('REMOTE_ADDR');
	}
...
{//inserta el mapa interactivo, y entraga variable $_GET['m'] del municipio que seleccino
//para usar esta funcion se espera en la pagina presente la var ?m=	* CONSIDERARLO
	...
	while ($df = $r2->fetch_array()) {//$df recorre la lista de las delegaciones
		...
		if (isset($_GET['mm'])) { // si hay seleccionado un MULTIPLE municipio
			...
			$seleccionados_ = explode(",", $seleccionados);
			$seleccionados_n = count($seleccionados_);
...
function MsgBox_Lite($mensaje, $link)
{
	...
	if ($tipo == 'ERROR') {
		Problema_create("PLATAFORMA", "MsgBox_Lite: " . $mensaje, $IdEmpleado);
	}
...
function DiaInabil_comentario($Cuando)
{
	require("config.php");
	$sql = "select * from diasinhabiles where Dia0='$Cuando'";
	$rc = $conexion->query($sql);
...
function EnviarNotificacion($usuario, $asunto, $contenido)
{
	...
	if ($usuario <> '') {
		// $npase = npase(FALSE);
		$sql = "INSERT INTO notificaciones
		(nitavu, asunto, contenido, entregar_fecha, nitavu_manda, grupal, lectura_fecha, lectura_hora, pie, firma_img, docdigital, no_oficio, prefijo, visto)
	VALUES
		('$usuario', '$asunto', '$contenido', '$fecha', '', '', '0000-00-00', '', '', '', '', '', '', 0)";
