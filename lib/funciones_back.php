...
function EdoCuentaFooter4($contrato, $IdDelegacion, $IdPrograma, $Folio){
	...
	FROM ___Vivienda_InformacionFinanciera WHERE numcontrato = '".$contrato."'";
	//echo $sql8.'<br>';
...
function VEdoCuentaFooter4($contrato, $IdDelegacion, $IdPrograma, $Folio, $OriginData){
	require("config.php");	$t1='';
	$sql8 ="SELECT *
	FROM ___Vivienda_InformacionFinanciera WHERE numcontrato = '".$contrato."' and OriginData='".$OriginData."'";
	//echo $sql8.'<br>';
...
function VEdoCuentaTablaDePagos($contrato, $IdDelegacion, $ContratoCancelado, $OriginData,$full, $cancelados){	
	...
	$sql7 = "
	SELECT * FROM Vivienda_PagosEdoCuenta_Check
 ...
 ";
	...
	SELECT	
	count(*) as n
	FROM Vivienda_PagosEdoCuenta_Check
	WHERE	(NumContrato = '".$contrato."' and OriginData='".$OriginData."')
...
if ($Registros <= 0 ){
	...
	// }
		...
		if ($Muestro == TRUE){
			...
			if ($value['SaldoError'] == '0'){ //si no tiene falla el registro		
					if ($c%2==0){
			...
			$t1 = $t1."</td>";
				...
				if ($c == 1){ 
					...
					//CARGOS
						...
						if ($value['Saldo'] == $value['SaldoCalculado']){ // lo comparamos para ver si se calculo bien
							$t1 = $t1.'<td align = "right" style="'.$styleTD.'">';
							// if($value['Saldo']<>0){
								$t1 = $t1.$StringStrike.Pesos($value['Saldo']).$StringStrike2;
							// }
							...
							// if($value['Saldo']<>0){
								$t1 = $t1.$StringStrike.Pesos($value['Saldo']).$StringStrike2;
								if ($value['Cancelado'] ==0){
								$t1 = $t1."<br><sup>[".$value['SaldoCalculado']."]</sup>";
								$t1 = $t1."<br><sup>* ".$value['SaldoError_desface']."</sup>";
								}
...
function VQR($contrato, $OriginData){
	...
	FROM
		___Vivienda_InformacionContratos 
	WHERE
...
function EdoCuenta_InfoFooter($contrato, $nitavu, $IdDelegacion, $myip){
	require("config.php");
	$sql9 = "select CONNECTIONPROPERTY('local_net_address') AS IP, BaseDeDatos, EstacionDeTrabajo from ___Vivienda_InformacionFinanciera where NumContrato = '".$contrato."'";
	//echo $sql9.'<br>';
...
function app_detalle($idapp, $nitavu=""){
	...
	if($f = $rc -> fetch_array())
	{
		...
		<table border=0 width=100%><tr>";
			$msg= $msg."<td width=20px align=left><a href='index.php'><img src='img/logo_vertical2.png' style='height: 50px;
			margin-top: -1px;'>"."</a></td>";
		...
		<div id='BloqueEspaciado' style='height:60px;'></div>
		";
