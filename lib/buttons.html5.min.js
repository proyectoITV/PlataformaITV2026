        ...
        action: function(a, c, d, b) {
            ...
            if (x.queryCommandSupported("copy")) { d.appendTo(c.table().container());
                ...
                b[0].select(); try { var p = x.execCommand("copy");
                    d.remove(); if (p) { c.buttons.info(c.i18n("buttons.copyTitle", "Copiado al Portapapeles"), c.i18n("buttons.copySuccess", { 1: "Copied one row to clipboard", _: "Copied %d rows to clipboard" }, a.rows), 2E3);
            ...
            c.buttons.info(c.i18n("buttons.copyTitle", "Copiado al Portapapeles"), p, 0);
            b[0].focus();
            b[0].select();
    ...
    D.ext.buttons.excelHtml5 = {
        ...
        action: function(a, c, d, b) {
                ...
                t, h, q = function(k) {
                    ...
                    for (var l = 0, z = k.length; l < z; l++) {
                        ...
                        A || ("number" === typeof k[l] || k[l].match && k[l].match(/^-?\d+(\.\d+)?$/) && !k[l].match(/^0\d+/) ?
                            A = y(f, "c", { attr: { t: "n", r: G }, children: [y(f, "v", { text: k[l] })] }) : (H = H.replace ? H.replace(/[\x00-\x09\x0B\x0C\x0E-\x1F\x7F-\x9F]/g, "") : H, A = y(f, "c", { attr: { t: "inlineStr", r: G }, children: { row: y(f, "is", { children: { row: y(f, "t", { text: H, attr: { "xml:space": "preserve" } }) } }) } })));
                        h.appendChild(A)
                    }
                    g.appendChild(h);
                    e++
            ...
            for (C = p.header.length; v < C; v++) q.appendChild(y(f,
                "col", { attr: { min: v + 1, max: v + 1, width: V(p, v), customWidth: 1 } }));
            q = a.xl["workbook.xml"];
