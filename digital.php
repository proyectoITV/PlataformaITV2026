	...
	echo "</div>";
		...
		if (isset($_GET['pes'])) {
			...
			{echo "<div id='itavu' class='pesta visible'>";
				...
				echo "<input type='submit' value='Seleccionar' class='Mbtn btn-default'></form>";
				...
				if (isset($_GET['folio']) and isset($_GET['prog']) and isset($_GET['del']))
				{
					...
					if($f = $rc -> fetch_array())
					{
						...
						if ($nivel<=2){// si es admin o superadmin		
						echo "<div><label>De clic en este boton para subir los que haya seleccionado</label><input type='submit' name='submit_subir' value='SUBIR' class='Mbtn btn-default'></div></form>";
						}
