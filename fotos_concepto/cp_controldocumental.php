...
if (sanpedro($id_aplicacion, $nitavu)==TRUE){
	...
	if(isset($_POST['comSolucionar'])){
		...
		//el estado en 2 nos dice que el caso ha sido terminado 
	...
	if(isset($_GET['editar'])){
		...
		remite="'.$remite.'", puesto="'.$puesto.'", dependencia="'.$dependencia.'", asunto="'.$asunto.'", descripcion="'.$descripcion.'",
		prioridad = "'.$prioridad.'", fecha_termino = "'.$fechaTermino.'"
		 WHERE id='. $numcaso.' ';*/
		 $sql = 'UPDATE cp_nuevosdocumentos SET fechaOficio="'.$fechaoficio.'", fecha="'.$fecha.'", oficioNumero="'.$ofnumero.'",
...
// echo "<a href='#documentoNew' rel='modal:open' class='btn btn-default' title='Registrar Caso'>";
...
echo "<a href='#documentoNew' rel='modal:open' class=' btn-g' title='Registrar Caso'>";
	echo "<table  width='100%'><tr><td valign='middle' align='center'>";
...
for ($i=0; $i < count($arr) ; $i++) { 
	if($nivel==1 || (soytitular($nitavu)!='FALSE'  and $dibuje==0) || ((soyColaborador($nitavu)=='TRUE') and (soyDptoturnado($arr[$i] ,$dpto)=='FALSE') and $dibuje==0) ){
		$dibuje=1;
		// echo "<a href='#myModalaAgregar' rel='modal:open' class='btn btn-default' title='Nuevo Número'>";
		// 	echo "<table  width='100%'><tr><td valign='middle' align='center'>";
		...
		// echo "<a href='#myModalaAgregar' rel='modal:open' class='btn-g2' title='Nuevo Número'>";
		// 	echo "<table  width='100%'><tr><td valign='middle' align='center'>";
		// 	echo "<img src='icon/nuevoNumero.png' >";
		// 	echo "</td>";
		// 	echo "<td valign='middle' align='center' style='color:white;' class='pc'>";
		...
		// echo "<a href='#docuementosRecientes' rel='modal:open' class='btn btn-tercero' title='Documentos Recientes' style='font-family: Compacta;'>";
		// 	echo "<table  width='100%'><tr><td valign='middle' align='center'>";
		...
		// echo "<a href='#docuementosRecientes' rel='modal:open' class='btn-g3' title='Documentos Recientes' style='font-family: Compacta;'>";
		// 	echo "<table  width='100%'><tr><td valign='middle' align='center'>";
		// 	echo "<img src='icon/docRecientes.png' >";
		// 	echo "</td>";
		// 	echo "<td valign='middle' align='center' style='color:white;' class='pc'>";
...
{
//Si soy titular 
// echo "<a href='cp_permisos.php' class='btn btn-tercero' title='Asignar permisos' style='font-family: Compacta;'>";
// 	echo "<table  width='100%'><tr><td valign='middle' align='center'>";
// 	echo "<img src='icon/docRecientes.png' >";
// 	echo "</td>";
// 	echo "<td valign='middle' align='center' style='color:white;' class='pc'>";
...
submenu_add2('#myModalaAgregar','req2.png','',' GENERAR NÚMERO');
...
//--------------------------------------------------------- 
echo "<div>";
echo "<a href='cp_miactividad.php?pes=finalizados' style='font-size:9pt; margin-left:90%;'>Actividad</a>";
echo "</div>";
...
// echo "<br><br>";
	...
	if (soytitular($nitavu)!='FALSE' || $nivel==1 ){
		...
		if ($count>0)		
		{
			...
			echo "<center>";
			echo "<h1>Mis Casos Pendientes de mi departamento:</h1>";
			...
			//if($r['prioridad']==1){
				...
				if ($r['Retraso']>=0){
					if ($r['Retraso']>=5 and $r['Retraso']<=30){
						echo "<hr><span class='pc tenue' style='font-size:9pt;  '><img src='icon/alerta_amarilla.png' style='width:15px;'>".$r['Retraso']." dias de atraso</span>";
					}
					if ($r['Retraso']>=31){
						echo "<hr><span class='pc tenue' style='font-size:9pt; '><img src='icon/alerta_roja.png' style='width:15px;'>".$r['Retraso']." dias de atraso</span>";
					}
				}
				...
				if($r['nitavuCaptura']==$nitavu){
					echo "<div id='columna2' style='vertical-align: middle; width:5%;'><center><a href='#modalEditarCaso".$r['id']."' rel='modal:open'><img id='imgcentrada' src='./icon/edit.png' style='width:15px;'></a></center></div></div></div>"	;
				}
				//MODAL EDITAR INFORMACION---------------------------------------------------------------------- 
				echo "<div id='modalEditarCaso".$r['id']."' class='modal'><h3>Modificar información del oficio: </h3>";
				echo "<form action='cp_controldocumental.php?editar=".$r['id']."' method='POST'  enctype='multipart/form-data'>";
					...
					while($res4 = $r4 -> fetch_array())    
					{
						...
						if($res4['fecha_termino']!='0000-00-00'){
							echo "<div id='divfechaTermino".$r['id']."' style='display:inline-block;'>";
								echo "<label>Fecha termino </label>  <input type='date' id='fechaTermino".$r['id']."' name='fechaTermino' value=".$res4['fecha_termino'].">";
							echo "</div>";
						}else{
					...
					/*	echo '<div><label>Prioridad</label>';
						...
						echo '<textarea style="height:20%;" name="descripcion">'.$res4['descripcion'].'</textarea></span>';
						//   echo '<br><br>'; 
						echo '<div><input class="btn btn-default" id="boton" type="submit" value="Guardar"></div>';
			...
			//---------------------------------------------------------------------------------------------------	
				...
				if($r['nitavuCaptura']==$nitavu){
					echo '<div id="colum2">';
						...
						echo "<button type='submit' class='btn btn-cancel' title='Haga clic para eliminar el caso' onclick=''> <img src='icon/delete.png' style='width:20px; height:20px;'> </button>"; 
						echo "</form>";
					echo '</div>';
				}
				...
				echo "<button type='submit' class='btn btn-default' title='Haga clic para dar el visto bueno' onclick=''> <img src='icon/ok.png' style='width:20px; height:20px;'> </button>"; 
				echo "</form>";
				echo "</td>";
				echo "<td>";
		...
		/*	}else if($r['prioridad']==2){
				...
				echo "</td>";
				echo "<td style='width:20px; border-top-style: none; '><center><a href='#modalEditarCaso".$r['id']."' rel='modal:open'><img src='./icon/edit.png' style='width:15px;'></center></a></td></table>"	;
			...
			//---------------------------------------------------------------------------------------------------	
				...
				echo "<button name='vobo' type='submit' class='btn btn-default' title='Haga clic para dar el visto bueno' onclick='deshabilitar()'> <img src='icon/ok.png' style='width:20px; height:20px;'> </button>"; 
				echo "</form>";
				echo "</td>";
				echo "<td>";
					echo "<center><p style='font-size:20pt; color:#6F7070; '>".actividaddelCaso($r['id'])."</p></center>";
...
if ($rc->num_rows>0)
{
	...
	echo "<div id=''><table class='tabla'>";
	echo "<th width='10%' COLSPAN='2'>Fecha</th>";
	echo "<th width='25%'>Asunto</th>";
	echo "<th width='7%'>Ver</th>";
  while($r = $rc -> fetch_array())    
  {
	echo "<tr>";
		echo "<td>".$r['id']."</td>";
		echo "<td>".$r['fecha']."</td>";
		echo "<td><b>".$r['asunto']."</b><span style='font-size:7pt'><br>".$r['descripcion']."</span><br>
		<span style='color:blue;'>Creado por: ".nombreDepartamento($r['idDptoCrea'])."</span></td>";
  ...
  echo "</div>";
	...
	echo "<div id='documentoNew' class='modal'><h3>Agregar nuevo oficio: </h3>";
   
    
		
			echo "<form action='cp_controldocumental.php' method='POST'  enctype='multipart/form-data'>";
			echo "<label style='font-size:8pt;'> *Si agregas fecha termino la plataforma
			te enviará notificaciones y correos diarios hasta el día de su vencimiento.</label>";
			 ...
			 <td></td>
			 <td> <label><input type="checkbox" id="fechaTer" name="fechaTer" value="fechaTer" onClick="mostrarFechaTermino()">Fecha termino</label></td></table></center>';
		 echo '<div><table style="width: 100%">
            <tr>
            <td align=right> <label>Fecha del Oficio</label> <input type="date" id="fechaOficio" name="fechaOficio" required>  </td>
            <td align=left>     </td>
            <td align=right>  <label>Fecha actual</label>  <input   style="background-color:#CCCCCC;" type="date" id="fecha" name="fecha" value='.$fecha.' required readonly>  </td>
            <td align=left>  </td>
            </tr>
			</table></div>';
      ...
      echo '<div><label>Referencia</label>';
	  echo '<input id="ofnumero" name="ofnumero" style="background-color:#FF9933;" title="Numero consecutivo de oficios de atencion al publico"/></div>';
	/*  echo '<div><label>Prioridad</label>';
	  echo '<select id="prioridad" name="prioridad" required>
				<option value="">Seleccione una prioridad</option>
      ...
      echo '<div><label>Remite</label>';
      echo '<input placeholder="Remite" id="remite" name="remite"></div>';
      echo '<div><label>Puesto</label>';
      echo '<input placeholder="Puesto" id="puesto" name="puesto"></div>';
      echo '<div><label>Dependencia</label>';
      echo '<input placeholder="Dependencia" id="dependencia" name="dependencia"></div>';
      echo '<div><label> Asunto</label>';
			echo '<input placeholder="Asunto" name="asunto" required></div>';		
			
				echo "<div style='display:none;' id='turnarDpto'>";			
			...
			echo "</div>";	
      echo '<span><label> Descripcion </label>';
      echo '<textarea style="height:20%;" name="descripcion"></textarea></span>';
      echo '<div><label>Archivo</label>';
      echo '<input id="myFile" name="myFile" type="file" accept=".pdf"></div>';
    //   echo '<br><br>'; 
      echo '<div><input class="btn btn-default" id="boton" type="submit" value="Guardar"></div>';
      echo '</form>';
		echo "</div>";
	...
	if (isset($_POST['departamento']) && isset($_POST['turnar'])){
					...
					if ($subida == "TRUE"){
						...
						if($conexion->query($sql) == TRUE){ 
							...
							if ($conexion->query($sql2) == TRUE){  
								...
								mensaje('Se ha registrado con éxito el nuevo documento.','cp_controldocumental.php');
		...
		if(isset($_POST['ofnumero']) and isset($_POST['asunto']) and isset($_POST['descripcion']) and !isset($_GET['editar'])){
			if(!empty($_FILES['myFile']['name']) != null){
				...
				if ($subida == "TRUE"){
					$sql = "INSERT INTO cp_nuevosdocumentos(idauto, id, fechaoficio, fecha, oficionumero, remite, puesto, dependencia,asunto, descripcion, nitavucaptura, iddptocrea,turnadoa,estado,baja, vobo, fecha_termino) 
					VALUES ('', '$idDocumento','$fechaOficio', '$fecha', '$ofnumero', '$remite', '$puesto', '$dependencia','$asunto','$descripcion','$nitavu','$dpto','$dpto',0,0,'','$fechaTermino')";
					if($conexion->query($sql) == TRUE){ 
						if($ofnumero==numeroOficioPublico(TRUE)){
							numeroOficioPublico(FALSE);
						}
						idDocumento(FALSE);
						$sql2 = "INSERT INTO cp_historialdocumentos(idinc, iddoc, numcaso, archivo, fecha, nitavusube, dptosube, dptoenviar, numoficio, activo, tipo,hora) 
						VALUES ('', '$numDocumento', '$idDocumento', '$myFile', '$fecha', '$nitavu', '$dpto','$dpto','$ofnumero',0,0,'$hora')";
						if ($conexion->query($sql2) == TRUE){  
							numdeDocumento(FALSE);
							historia($nitavu,'cp_Agregó un nuevo caso llamado: '.$ofnumero.' con Id: '.$idDocumento);
							historia($nitavu,'cp_Agregó un nuevo caso  Id: '.$idDocumento.' Archivo: '.$myFile);
				...
				$numDocumento = numdeDocumento(TRUE);
					$sql = "INSERT INTO cp_nuevosdocumentos(idauto, id, fechaoficio, fecha, oficionumero, remite, puesto, dependencia,asunto, descripcion, nitavucaptura, iddptocrea, turnadoa, estado, baja, vobo, fecha_termino) 
					VALUES ('', '$idDocumento','$fechaOficio', '$fecha', '$ofnumero', '$remite', '$puesto', '$dependencia',
					'$asunto','$descripcion','$nitavu','$dpto','$dpto',0,0,'','$fechaTermino')";
					if($conexion->query($sql) == TRUE){ 
						/*if($ofnumero==numeroOficioPublico(TRUE)){
							numeroOficioPublico(FALSE);
						}*/
...
//--------------------------------------------------------- 
echo "<div id='docuementosRecientes' class='modal'><h3>Lista de documentos recientes: </h3>";
	...
	{	
		...
		echo "<span class='tchico'><b>".$co['nombre']."</b></span><br></td>";
			...
			
			{
				...
				echo "<button type='submit' name='btnCancelar' id='btnCancelar' class='btn btn-tercero' title='Cancelar' >";
				echo "<img src='icon/x.png' class='btn_cat_img' >";
				echo "</button>";
				echo "<a href='?DocId=".$co['Id']."'  title='Haz clic aqui para indicar que ya usaste este oficio' class='btn btn-secundario' style='margin-left: 5px;'><img src='icon/reveri.png' style='width:25px;'></a>";
				echo "</div>";
				echo "</td>";
			
		  }

		//echo "<td><a href='?DocId=".$co['Id']."'  title='Haz clic aqui para indicar que ya usaste este oficio'class='btn btn-secundario'><img src='icon/reveri.png' style='width:25px;'></a></td>";
		echo "</form>";	
		echo "</tr>";
	}
echo "</table>";
