...
$id_aplicacion ="ap37"; //Id de la aplicacion a cargar
$nivel = aplicacion_nivel($id_aplicacion, $nitavu);
if (sanpedro($id_aplicacion, $nitavu)==TRUE){
echo "<h5>".app_detalle($id_aplicacion)."</h5>";
historia ($nitavu,"Entro a ver las Reservas Territoriales Regularizadas (app: ".$id_aplicacion.")");

	echo "<div id='kmz'>";
	//echo $nivel;

	if ($_GET['m']=="") {// si no se ha seleccionado el municipio mostrar lista de actualizacion y png
		echo "<img src='img/reserva.png'>";
		echo "<div id='kmz_str'>";
		...
		{
		$archivo = "kmz/reg/".$m['IdMunicipio'].".kmz";
	...
	<div id='grafica'><div id='chart_div'></div></div>
		...
		$intento="";
		$archivo = "kmz/reg/".$_GET['m'].".kmz";		
		if (file_exists($archivo)){$intento="SI";}
		
		$archivo2 = "kmz/reg/".$_GET['m'].".kml";		
		if (file_exists($archivo2)){$intento="SI"; $archivo = $archivo2;}
		

		if ($intento == "SI"){
			historia ($nitavu,"Entro a ver las Reservas Territoriales Regularizadas del municipio de ".municipio_nombre($_GET['m'])." (app: ".$id_aplicacion.")");
			...
			var ctaLayer = new google.maps.KmlLayer({url:'https://plataformaitavu.tamaulipas.gob.mx/".$archivo."',map: map});
			}
			</script>
			...
			<a href='kmz_reg.php?m=' class='btn btn-default'>Regresar</a>
			<a href='kmz.php?m=".$_GET['m']."' class='btn btn-default'>Ver Reserva</a>


			";
		} else {
			historia ($nitavu,"Entro a ver las Reservas Territoriales Regularizadas del municipio de ".municipio_nombre($_GET['m'])." Sin encontrar resultados (app: ".$id_aplicacion.")");
		...
		if ($nivel=='02'){
			...
			<div>
				...
				<form action="kmz_reg.php?m='.$_GET['m'].'" method="POST" enctype="multipart/form-data">
				<div>
				...
				<div>
					<label>* Preste especial atencion al seleccionar el archivo correspondiente, ya que se reescribiran datos del municipio</label>			
					<input type="submit" name="submit_importar" value="Importar" class="btn btn-default">
			...
			if(isset($_POST['submit_importar'])){
				...
				if ($_POST['tipo']=="kmz"){
						$target_path = "kmz/reg/".$_GET['m'].'.'.'kmz';
				} else {$target_path = "kmz/reg/".$_GET['m'].'.'.'kml';}
