...
if (sanpedro($id_aplicacion, $nitavu)==TRUE){    
	...
	if (isset($_GET['personal'])){ //MODULO CARTERO //--------------------------------------------------------
		...
		echo "<div><input type='submit' class='btn btn-default' name='CarteroGuardar' value='Guardar (NPase ".$npase_reservado.")'></div>";
			...
			if ($f['rechazada']=='TRUE'){
				...
				echo $f['Recibido'];
					...
					if ($f['Recibido']==''){
						...
						echo "<input type='submit' value='Guardar' name='recibido_g' class='btn btn-tercero'>";
						echo "</form>";
					}
						...
						//echo "<a href='".$src."&VFclose=".$origen."' ' title='Imprimir' class='btn btn-default'>Imprimir</a>";
				...
				echo "</td>";
				echo "<td>";
				echo "<a href='embarques_cartero_print.php?nuc=".$nitavu."&guia=".$f['NPase']."' class='btn btn-default'>imprimir</a>";
				echo "</td>";
				echo "</tr>";
	...
	if (isset($_GET['enviar'])){//si le dieron clic al boton registrar envio
		...
		echo "<div><label class='pc'>No olvide, incluir en el paquete el Ticket de Envio</label><input type='submit' name='guardar_envio' value='Asignar Envio' class='btn btn-default'></div><hr>";
			...
			$sql="UPDATE embarques_guias SET destino='".$_POST['destino']."', descripcion='".$_POST['descripcion']."' WHERE guia='".$_POST['guia']."'";
					if ($conexion->query($sql) == TRUE)
						...
						$contenido = "<p>Buen dia <b>"."</b>:</p>";
							...
							"."* Si no te ha llegado el correo de esta notificación, verifica que este activado. Te aconsejamos tambien configures tu correo en tu telefono, asi te informaremos de este y otro tipo de notificaciones referentes al Instituto. Cualquier duda, comunicarse al Dpto. de Informatica"."</p>";
	...
	if (isset($_GET['foraneo']) and ($nivel==1)){//si le dieron clic al boton registrar envio
		...
		echo "<div><label>fotografia (Opcional)</label><input type='file' name='foto'></div>";
		echo "<div><label class='pc'>No olvide, incluir en el paquete el Ticket de Envio</label><input type='submit' name='guardar_envioforaneo' value='Guardar' class='btn btn-default'></div><hr>";
	...
	if (embarque_recibido($_GET['recibir'])==''){
		...
		echo "<div><label class='pc'></label><input type='submit' name='guardar_envio' value='Registrar Recepcion' class='btn btn-default'></div>";
    ...
    if (isset($_GET['guias']) and  ($nivel=='1') and !isset($_GET['foraneo'])){
		...
		if (isset($_GET['act'])){		
			echo "<div>"."<input type='submit' name='actualizar_guia' value='Actualizar' class='btn btn-default'>"."</div>";	
		} else {
			echo "<div>"."<input type='submit' name='guardar_guia' value='Registrar' class='btn btn-default'>"."</div>";
		}
	...
	echo "<h5><b>Otorgar permiso para usar esta aplicacion</b></h5>";
	...
	echo "<input type='submit' value='Dar permiso' class='btn btn-default' name='submit_todos'>";
	echo "</div>";
	echo "</form>";
	echo "</div>";
	...
	echo "<h1>Los siguientes empleados actualmente tienen acceso a la Aplicacion</h1>";
	echo "<table class=tabla>";
...
//area sin permisos
	...
	//o es un titular de departamento: info disponible para titular o nivel 2
		...
		while($f = $r2 -> fetch_array()){
			...
			{
				echo "<a target:_blank href='embarques_reporte.php?guia=".$f['guia']. "&nuc=$nitavu'><img src='icon/embarques_print.png' style='width:20px; height:20px; background-color:gray; border-radius:3px;'></a>";
			}
...
//					echo $sql;
		...
		$sql = "SELECT * FROM embarques_guias WHERE destino<>'' and origen='".nitavu_dpto($nitavu)."' order by recibido";
		//echo $sql;
		$r2 = $conexion -> query($sql);
		...
		while($f = $r2 -> fetch_array()){
			...
			if ($f['visto_paraimprimir']=='0' and $f['recibido']=='' ){
				...
				echo "<input type='submit' value='Actualizar descripcion' class='btn btn-default'>";
				echo "</form>";
			...
			if ($f['destino']==''){
				echo "<a href='?guias=&enviar=".$f['guia']. "'><img src='icon/embarques_enviar.png' style='width:20px; height:20px; background-color:gray; border-radius:3px;'></a>";
					...
					echo "<a target:_blank href='embarque_reporte.php?guia=".$f['guia']. "&nuc=$nitavu'><img src='icon/embarques_print.png' style='width:20px; height:20px; background-color:gray; border-radius:3px; margin:5px;'></a>";
...
if (!isset($_GET['guias']) AND !isset($_GET['permisos'])  AND !isset($_GET['estadistica']) AND !isset($_GET['enviar']) AND !isset($_GET['enviarforaneo'])  AND !isset($_GET['personal']) ) {
	...
	// echo $sqlx;
		...
		while($f = $r2 -> fetch_array()){
			...
			if ($f['destino']==''){
				//echo "<a href='?enviar=".$f['guia']. "'><img src='icon/embarques_enviar.png' style='width:20px; height:20px; background-color:gray; border-radius:3px;'></a>";
					...
					echo "<a target:_blank href='embarque_reporte.php?guia=".$f['guia']. "&notoken&nuc=$nitavu'><img src='icon/embarques_print.png' style='width:20px; height:20px; background-color:gray; border-radius:3px;'></a>";
		...
		<form action='' method='POST' enctype='multipart/form-data'>
			...
			<input type='submit' value='Leer codigo QR' name='qr_form' class='btn btn-tercero'>
		</form>";
...
//--------------------------------------------------------- paquetes por recibir
if (!isset($_GET['guias']) AND !isset($_GET['permisos'])  AND !isset($_GET['estadistica']) AND !isset($_GET['enviar']) AND !isset($_GET['enviarforaneo'])  AND !isset($_GET['personal'])) {
		$r2 = $conexion -> query("SELECT * FROM embarques_guias WHERE recibido<>'' and destino='".nitavu_dpto($nitavu)."' and recibio_fecha<>''");
		...
		while($f = $r2 -> fetch_array()){
			...
			if ($f['destino']==''){
				echo "<a href='?guias=&enviar=".$f['guia']. "'><img src='icon/embarques_enviar.png' style='width:20px; height:20px; background-color:gray; border-radius:3px;'></a>";
					...
					echo "<a href='embarque_reporte.php?guia=".$f['guia']. "&nuc=$nitavu'><img src='icon/embarques_print.png' style='width:20px; height:20px; background-color:gray; border-radius:3px;'></a>";
...
//PARA GENERAR PERMISOS O SER TITULAR DE UN PDTO.
	...
	<div id='chart_div' ></div>
				...
				";
	echo $grafica;
	echo "<br><a href='?estadistica=all' class='btn btn-default'>Mostrar Grafica de todos los tiempos</a>";
	}
