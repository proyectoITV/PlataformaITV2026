...
if (sanpedro($id_aplicacion, $nitavu)==TRUE){
	...
	if($nivel == 1) {//ADMINISTRADOR GENERAL: Aprueba todo--------------
		...
		while($f = $rc -> fetch_array()) {
			...
			echo "<table border='0' class='tbl_dir'><tr>";
					...
					echo "<button class='btn btn-default' onclick=OK_pase('".$f['id']."','".$nitavu."');>";
					...
					echo "<button class='btn btn-cancel' onclick=X_pase('".$f['id']."','".$nitavu."');>";
	...
	echo "<h1>Puede aprobar solo a tu dpto.</h1>";
		echo "<div id='r'>";
		$sql = "SELECT * FROM empleados_salidas_temporal WHERE (autorizo_nitavu='' AND fecha>='".$fecha."' AND dpto='".nitavu_dpto($nitavu)."' and nitavu<>'".$nitavu."')";	
		//echo $sql;
		$rc= $conexion -> query($sql);
		while($f = $rc -> fetch_array()) {
			echo "<div  class='pase_elementos'>";				
