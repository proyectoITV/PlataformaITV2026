...
<?php
  ...
  if (isset($_GET['curp'])){
    ...
    $ConsultarRENAPO = 0;    //Cambiar esta opcion para salir a consultar a RENAPO valores [0, 1]

    if ($ConsultarRENAPO == 1) {
        $ResultadoDelCURP = CURP($CURP, $nitavu); 
        $array = json_decode($ResultadoDelCURP, true);
    
        if (is_array($array) && isset($array['data'])) {
            $persona = $array['data'];
            $Beneficiario = $persona['nombres']." ".$persona['apellido1']." ".$persona['apellido2'];
            $FechaNacimiento = ConvertirFechaParaMySQL($persona['fechNac']);
            $Edad = CalcularEdad($FechaNacimiento);
            $Sexo = $persona['sexo'];
            $DatosExtras = $DatosExtras."
            Edad: <b>". $Edad ." años</b>, Fecha de Nacimiento: ".$persona['fechNac']." <br>
            Nacionalidad: ".$persona['nacionalidad'].", Nacio en <b> ".$persona['nombreEntidadNac']."</b><br>
            Estatus del CURP: ".$persona['statusCurp']."";
            ...
            echo "La variable no es un array o es nula (posiblemente por un error al decodificar JSON).\n";
    ...
    //     echo "</select></label>";             
    //     echo "<label>Fecha Nacimiento:<input type='date' name='FechaNacimiento' id='FechaNacimiento' value=''></label>";
    //       echo "<label>Edad:<input type='number' name='Edad' id='Edad' value='' disabled></label>";
    //     echo "</center></div>";*/
    // }
